...
首页可选普通默契卡或“一起听卡片”。创建者可以为每个自定义字段选择歌手、专辑、歌曲或纯自定义类型；三类音乐搜索都优先使用 QQ 音乐。
...
- 一起听卡片的歌手、专辑和歌曲搜索优先由 Worker 代理 QQ 音乐。歌手无结果时回退 TheAudioDB；专辑和歌曲无结果或上游异常时，浏览器先查询 iTunes 中国大陆目录，再自动回退香港目录。查询不区分大小写，输入至少两个字符后显示结果。
- 已发布内容可以撤回修改并重新发布；已经创建的公开分享仍是独立快照，不会被后续修改覆盖。
- 数据库只保存外部封面 URL 和商店详情 URL；封面展示和 Canvas 导出通过限定 QQ 音乐、TheAudioDB 与 `mzstatic.com` 域名的同源代理完成，不复制到 R2。
- 每个房间默认最多允许 50 位二号加入，可通过 `MAX_ROOM_GUESTS` 调整（服务端限制 1–500）。
