...
首页可选普通默契卡或“一起听卡片”。创建者可以为每个自定义字段选择歌手、专辑、歌曲或纯自定义类型；音乐检索统一使用 TheAudioDB 的目录与图片数据。
...
- 一起听卡片由 Worker 代理 TheAudioDB 歌手、专辑和歌曲搜索；查询不区分大小写，输入至少两个字符后显示结果。TheAudioDB 的部分歌曲记录没有封面时，仍可保存歌曲文字信息。
